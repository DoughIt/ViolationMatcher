package cn.edu.fudan.violationtracker.util.stat;
...
import cn.edu.fudan.violationtracker.util.StringsUtil;
...
public class LogicalStatementRule extends VoidVisitorAdapter<Object> {
    ...
    private void checkAnchor() {
        ...
        final List<TwoValue<String, Integer>> anchors = JavaAstParserUtil.findMethodNameAndOffsetList(cu, beginLines, beginLines);
        int count = 0;
        for (int idx : idxs) {
            logicalStatements.get(idx).setAnchorName( anchors.get(count).getFirst());
            logicalStatements.get(idx).setAnchorOffset( anchors.get(count++).getSecond());
        }
    }
